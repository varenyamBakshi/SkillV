<?php
/**
 * BuilderPress SiteOrigin widget class
 *
 * @version     1.0.0
 * @author      ThimPress
...
if ( ! class_exists( 'BuilderPress_SO_Widget' ) ) {
	/**
	 * Class BuilderPress_SO_Widget
	 */
	abstract class BuilderPress_SO_Widget extends WP_Widget {
		...
		/**
		 * BuilderPress_SO_Widget constructor.
		 */
		public function __construct() {
			...
			$widget_options = array(
				...
				'panels_groups' => array( 'builderpress_so_widgets' )
			);

			$options            = $config_class::$options;
			$this->form_options = BuilderPress_SO_Mapping::mapping_options( $options );//if($this->config_class == 'BuilderPress_Config_Countdown'){echo '<pre>';print_r($this->form_options);die;}
			...
			$this->base_folder = BUILDER_PRESS_INC . "elements/general/$this->base";
			...
			parent::__construct( 'thim-' . $this->base, $name, $widget_options, $control_options );
		}
		...
		public function widget( $args, $instance ) {
			...
			do_action( 'builder-press/before-element-template', $this->id_base );

			// include template
			echo ent2ncr( $args['before_widget'] );
			builder_press_get_template( $this->base, array( 'params' => $params ), $this->group . '/' . $this->base . '/tpl/' );
		...
		public function _handle_variables( $instance ) {
			...
			do_action( 'builder-press/so/handle-variables', $instance );
...
//					}
		...
		public function form( $instance ) {
			...
			?>
            <div class="thim-widget-form thim-widget-form-main thim-widget-form-main-<?php echo esc_attr( $class_name ) ?>"
			...
			<?php if ( ! empty( $this->widget_options['help'] ) ) : ?>
                <a href="<?php echo esc_url( $this->widget_options['help'] ) ?>"
                   ...
                   target="_blank"><?php esc_attr_e( 'Help', 'builderpress' ) ?></a>
			<?php endif; ?>
            ...
            <script type="text/javascript">
                (function ($) {
                    ...
                    if (!$('#builder-presss-so-widget-admin-css').length && $.isReady) {
                        alert('<?php esc_attr_e( 'Please refresh this page to start using this widget.', 'builderpress' ) ?>')
                    }
                })(jQuery);
            </script>
			<?php
		...
		function backend_scripts() {
			...
			if ( ! wp_script_is( 'builder-press-so-widget-admin' ) ) {
				wp_enqueue_style( 'wp-color-picker' );
				wp_enqueue_script( 'wp-color-picker-alpha', BUILDER_PRESS_URL . 'inc/builders/siteorigin/assets/js/wp-color-picker-alpha.min.js', array( 'wp-color-picker', ), '', true );

				wp_enqueue_media();

				wp_enqueue_style( 'builder-presss-so-widget-admin', BUILDER_PRESS_URL . '/inc/builders/siteorigin/assets/css/widget-admin.css', array( 'media-views' ), BUILDER_PRESS_VER );
				wp_enqueue_script( 'builder-press-so-widget-admin', BUILDER_PRESS_URL . '/inc/builders/siteorigin/assets/js/widget-admin.min.js', array(
					'jquery',
					'jquery-ui-sortable',
					'editor'
				), BUILDER_PRESS_VER, true );

				wp_localize_script( 'builder-press-so-widget-admin', 'soWidgets', array(
					'sure' => __( 'Are you sure?', 'builderpress' )
				) );
			}
		}
		...
		function render_field( $name, $field, $value, $repeater = array(), $is_template = false ) {
			...
			if ( $field['type'] != 'repeater' && $field['type'] != 'checkbox' && $field['type'] != 'separator' && ! empty( $field['label'] ) ) {
                ...
                <label for="<?php echo esc_attr( $field_id ) ?>"
					...
					if ( ! empty( $field['optional'] ) ) {
						echo ' <span class="field-optional">(' . __( 'Optional', 'builderpress' ) . ')</span>';
					}
					?>
                </label>
				<?php
			...
			switch ( $field['type'] ) {
				...
				case 'media':
					if ( version_compare( get_bloginfo( 'version' ), '3.5', '<' ) ) {
						printf( __( 'You need to <a href="%s">upgrade</a> to WordPress 3.5 to use media fields', 'builderpress' ), admin_url( 'update-core.php' ) );
						break;
					}
					if ( ! empty( $value ) ) {
						if ( is_array( $value ) ) {
					...
					$choose_title  = empty( $args['choose'] ) ? __( 'Choose Media', 'builderpress' ) : $args['choose'];
					$update_button = empty( $args['update'] ) ? __( 'Set Media', 'builderpress' ) : $args['update'];
                    ...
                    <div class="media-field-wrapper">
                        ...
                        <a href="#" class="media-remove-button"><?php esc_attr_e( 'Remove', 'builderpress' ) ?></a>
                    </div>
				...
				case 'posts' :
                    ...
                    <a href="#" class="ob-select-posts button button-secondary">
						...
						<?php esc_attr_e( 'Build Posts Query', 'builderpress' ) ?>
                    </a>
					<?php
					break;
				...
				case 'repeater':
                    ...
                    <div class="thim-widget-field-repeater"
                        ...
                        <div class="thim-widget-field-repeater-add"><?php esc_attr_e( 'Add', 'builderpress' ) ?></div>
                    </div>
					<?php
					break;
				...
				case 'so_link':
                    ...
                    <div class="select-link-popup-button">
                        ...
                        <a href="#" class="button"><?php _e( 'Select URL', 'builderpress' ); ?></a>
                        <span class="link-title-label"><?php _e( 'Title: ', 'builderpress' ); ?></span>
                        <span class="link-title"><?php echo esc_html( $title ); ?></span>
                        <span class="link-url-label"><?php _e( 'URL: ', 'builderpress' ); ?></span>
                        <span class="link-url"><?php echo esc_url( $url ) . ' ' . esc_attr( $target ) . ' ' . esc_attr( $rel ); ?></span>
                    </div>
					<?php
					break;
                ...
                case 'sobp_datetime':
                    ...
                    <script>jQuery('.bp-datetimepicker').datetimepicker({
                            beforeShow: function () {
                                if (!jQuery('.bpso_datepicker_wrapper').length) {
                                    jQuery('#ui-datepicker-div').wrap('<span class="bpso_datepicker_wrapper"></span>');
                                }
                            }
                        });</script>
                    <?php
				...
				default:
					...
					esc_html_e( 'Unknown Field', 'builderpress' );
					break;
			}
