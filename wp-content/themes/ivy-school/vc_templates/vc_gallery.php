...
if ( 'nivo' === $type ) {
	...
	$slides_wrap_end = '</ul>';
}  elseif('thim_slider' === $type){
        $slides_wrap_start = '<div class="thim-service-slider"> <div class="wrapper-thim-slider">';
        $slides_wrap_end = '</div> </div>';

        // start 1
        $thim_start= '<div class="service-item">';
        $thim_end = '</div>';

        // start 2
        $thim_start1= '<div class="service-item-image">';
        $thim_end1 = '</div>';
...
foreach ( $images as $i => $image ) {
    ...
    if('thim_slider' === $type) {
        $thim_gal_images .= $el_start . $link_start . $thumbnail . $link_end . $el_end;
    }else {
...
if('thim_slider' === $type) {
    $output .=  $slides_wrap_start;
        $output .=  $thim_start;
            $output .=  $thim_gal_images   ;
        $output .=  $thim_end;

        $output .=  $thim_start1;
            $output .= $thim_gal_images ;
        $output .=  $thim_end1;
    $output .=  $slides_wrap_end;
