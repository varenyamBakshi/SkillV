...
<div id="learn-press-become-teacher-form" class="become-teacher-form learn-press-form <?php if( $user && ! $user instanceof LP_User_Guest && !learn_press_become_teacher_sent() ) echo 'allow'; else echo 'block-fields';?>">
    ...
    <h4 class="teacher-title"><?php esc_html_e( 'Register to become an Intructor', 'ivy-school' ); ?></h4>

    <?php if ( ! $user || $user instanceof LP_User_Guest ) { ?>
        <p class="message message-info"><?php printf( __( 'You have to <a class="login" href="%s">login</a> to fill out this form', 'ivy-school' ), add_query_arg( 'redirect_to', get_permalink(), thim_get_login_page_url() ) ); ?></p>
    <?php } ?>
    ...
    <div class="more-info">
        <?php printf( __( '* Furthermore, while interacting with the Eduma school website, I agree to abide by <a href="%s">Ivy school Terms of Use.</a>', 'ivy-school' ), get_permalink() );?>
    </div>
