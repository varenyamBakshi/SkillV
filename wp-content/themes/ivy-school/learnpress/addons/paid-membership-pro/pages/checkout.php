...
<div id="pmpro_level-<?php echo esc_attr($pmpro_level->id); ?>">
	<form id="pmpro_form" class="pmpro_form" action="<?php if ( ! empty( $_REQUEST['review'] ) ) {
		...
		<?php if ( $pmpro_msg ) {
			?>
			<div id="pmpro_message" class="pmpro_message <?php echo esc_attr($pmpro_msgt) ?>"><?php echo ent2ncr($pmpro_msg) ?></div>
			<?php
		} else {
			?>
		...
		<?php if ( $pmpro_review ) { ?>
			<p><?php echo esc_html__( 'Almost done. Review the membership information and pricing below then <strong>click the "Complete Payment" button</strong> to finish your order.', 'ivy-school' ); ?></p>
		<?php } ?>
		...
		<table id="pmpro_pricing_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
			...
			<tr>
				<th>
					<span class="pmpro_thead-name"><?php echo esc_html__( 'Membership Level', 'ivy-school' ); ?></span>
					<?php if ( count( $pmpro_levels ) > 1 ) { ?><span class="pmpro_thead-msg">
						<a href="<?php echo pmpro_url( "levels" ); ?>"><?php echo esc_html__( 'change', 'ivy-school' ); ?></a>
						</span><?php } ?>
				</th>
			</tr>
			</thead>
			...
			<tr>
				<td>
					<p>
						<?php printf( __( 'You have selected the <strong>%s</strong> membership level.', 'ivy-school' ), $pmpro_level->name ); ?>
					</p>
					...
					if ( ! empty( $level_description ) ) {
						echo ent2ncr($level_description);
					}
					?>
					...
					<?php if ( $pmpro_show_discount_code ) { ?>
						...
						<?php if ( $discount_code && ! $pmpro_review ) { ?>
							<p id="other_discount_code_p" class="pmpro_small">
								<a id="other_discount_code_a" href="#discount_code"><?php echo esc_html__( 'Click here to change your discount code', 'ivy-school' ); ?></a>.
							</p>
						<?php } elseif ( ! $pmpro_review ) { ?>
							<p id="other_discount_code_p" class="pmpro_small"><?php echo esc_html__( 'Do you have a discount code?', 'ivy-school' ); ?>
								<a id="other_discount_code_a" href="#discount_code"><?php echo esc_html__( 'Click here to enter your discount code', 'ivy-school' ); ?></a>.
							</p>
						<?php } elseif ( $pmpro_review && $discount_code ) { ?>
							<p>
								<strong><?php echo esc_html__( 'Discount Code', 'ivy-school' ); ?>:</strong> <?php echo esc_html($discount_code) ?>
							</p>
						<?php } ?>
		...
		<?php if ( ! $skip_account_fields && ! $pmpro_review ) { ?>
			<table id="pmpro_user_fields" class="pmpro_checkout" width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<tr>
					<th>
						<span class="pmpro_thead-name"><?php echo esc_html__( 'Account Information', 'ivy-school' ); ?></span>
						<span class="pmpro_thead-msg"><?php echo esc_html__( 'Already have an account?', 'ivy-school' ); ?>
							<a href="<?php echo add_query_arg( 'redirect_to', pmpro_url( "checkout", "?level=" . $pmpro_level->id ), thim_get_login_page_url() ); ?>"><?php echo esc_html__('Log in here', 'ivy-school' );?></a>
					</th>
				</tr>
				</thead>
				<tbody>
				<tr>
					<td>
						...
						<div class="pmpro_hidden">
							...
							<strong><?php echo esc_html__( 'LEAVE THIS BLANK', 'ivy-school' ); ?></strong>
						</div>
		...
		<?php if ( pmpro_getGateway() == "paypal" && empty( $pmpro_review ) && true == apply_filters( 'pmpro_include_payment_option_for_paypal', true ) ) { ?>
			<table id="pmpro_payment_method" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( ! $pmpro_requirebilling ) { ?>style="display: none;"<?php } ?>>
				<thead>
				<tr>
					<th><?php echo esc_html__( 'Choose your Payment Method', 'ivy-school' ); ?></th>
				</tr>
				</thead>
				<tbody>
				<tr>
		...
		if ( $pmpro_include_billing_address_fields ) {
			...
			<table id="pmpro_billing_address_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( ! $pmpro_requirebilling || apply_filters( "pmpro_hide_billing_address_fields", false ) ){ ?>style="display: none;"<?php } ?>>
				<thead>
				<tr>
					<th><?php echo esc_html__( 'Billing Address', 'ivy-school' ); ?></th>
				</tr>
				</thead>
				<tbody>
				<tr>
		...
		if ( $pmpro_include_payment_information_fields ) {
			...
			<table id="pmpro_payment_information_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0" <?php if ( ! $pmpro_requirebilling || apply_filters( "pmpro_hide_payment_information_fields", false ) ) { ?>style="display: none;"<?php } ?>>
				...
				<tr>
					<th>
						<span class="pmpro_thead-name"><?php echo esc_html__( 'Payment Information', 'ivy-school' ); ?></span>
						<span class="pmpro_thead-msg"><?php printf( esc_html__( 'We Accept %s', 'ivy-school' ), $pmpro_accepted_credit_cards_string ); ?></span>
					</th>
				</tr>
				</thead>
				<tbody>
					...
					<td>
						...
						if ( $pmpro_show_cvv ) { ?>
							<div class="pmpro_payment-cvv">
								<label for="CVV"><?php echo esc_html__( 'CVV', 'ivy-school' ); ?></label>
								...
								<small>(<a href="javascript:void(0);" onclick="javascript:window.open('<?php echo pmpro_https_filter( PMPRO_URL ) ?>/pages/popup-cvv.html','cvv','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=yes, resizable=yes, width=600, height=475');"><?php echo esc_html__( "what's this?", 'ivy-school' ); ?></a>)
								</small>
							</div>
						<?php } ?>
			...
			<table id="pmpro_tos_fields" class="pmpro_checkout top1em" width="100%" cellpadding="0" cellspacing="0" border="0">
				...
				<tr class="odd">
					<td>
						...
						<label class="pmpro_normal pmpro_clickable" for="tos"><?php printf( esc_html__( 'I agree to the %s', 'ivy-school' ), $tospage->post_title ); ?></label>
					</td>
				</tr>
				</tbody>
			</table>
			...
			<?php } else { ?>
				...
				if ( $pmpro_checkout_default_submit_button ) {
					...
					<input type="submit" class="pmpro_btn pmpro_btn-submit-checkout" value="<?php if ( $pmpro_requirebilling ) {
						echo esc_html__( 'Submit and Check Out', 'ivy-school' );
					} else {
						echo esc_html__( 'Submit and Confirm', 'ivy-school' );
					} ?> &raquo;" />
				</span>
					<?php
				}
