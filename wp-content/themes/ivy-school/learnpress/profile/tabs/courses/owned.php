...
<div class="learn-press-subtab-content">
    ...
    <h3 class="profile-heading">
		<?php echo esc_html__( 'My Courses', 'ivy-school' ); ?>
    </h3>

	<?php if ( $filters = $profile->get_own_courses_filters( $filter_status ) ) { ?>
        <ul class="lp-sub-menu">
			<?php foreach ( $filters as $class => $link ) { ?>
                <li class="<?php echo esc_attr($class); ?>"><?php echo ent2ncr($link); ?></li>
			<?php } ?>
        </ul>
	<?php } ?>

	<?php if ( ! $query['total'] ) {
		learn_press_display_message( esc_html__( 'No courses!', 'ivy-school' ) );
	} else { ?>

        <ul class="learn-press-courses profile-courses-list row grid-courses">
			<?php
			global $post;
