...
class Tax_Meta_Class {
  ...
  public function check_field_time() {
    ...
    if ( $this->has_field( 'time' ) && $this->is_edit_page() ) {
      wp_enqueue_script( 'at-timepicker', $this->SelfPath . '/js/jquery-ui-timepicker-addon.min.js', array( 'tmc-jquery-ui' ),false,true );
    }    
  }
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $c = 0;
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        <span class="at-re-toggle"><img src="';
           ...
           echo '" alt="'.esc_attr__('Edit','ivy-school').'" title="'.esc_attr__('Edit','ivy-school').'"/></span> 
        <img src="';
        if ($this->_Local_images){
        ...
        echo '" alt="'.esc_attr__('Remove','ivy-school').'" title="'.esc_attr__('Remove','ivy-school').'" id="remove-'.$field['id'].'"></div>';
        $c = $c + 1;
    ...
    echo '" alt="'.esc_attr__('Add','ivy-school').'" title="'.esc_attr__('Add','ivy-school').'" id="add-'.$field['id'].'"><br/></div>';
    ...
    ";
    wp_add_inline_script( 'jquery-tax-meta', $script_tax_meta );
    ...
    ";
    wp_add_inline_style( 'css-tax-meta', $style_tax_meta );
  ...
  public function show_field_image( $field, $meta ) {
    ...
    echo "<input type='hidden' name='{$name}[url]' value='{$value['url']}'/>";
    if ($has_image)
    echo "<input class='{$multiple} button  simplePanelimageUploadclear' id='{$id}' value='".esc_html__('Remove Image', 'ivy-school')."' type='button'/>";
    else
    echo "<input class='{$multiple} button simplePanelimageUpload' id='{$id}' value='".esc_html__('Upload Image', 'ivy-school')."' type='button'/>";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function footer_js(){
      ...
      ";
      wp_add_inline_script( 'mete-footer-add-custom-js', $add_custom_meta_js );
  }
